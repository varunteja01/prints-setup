...
const InvoiceFooter = ({ footer, show_total = false, page_number }) => {
  return (
    <View style={styles.container}>
      ...
      <Text style={{ ...styles.total, width: '30%' }}>TOTAL</Text>
      <Text style={{ ...styles.total, width: '10%' }}>
        :{'  '}
        {show_total
          ? footer?.gross_total % 1 != 0
            ? `Rs. ${numberFormat(footer?.gross_total)}`
            : `Rs. ${numberFormat(footer?.gross_total)}.00`
          : `Continued... ${page_number + 1}`}
      </Text>
    </View>
  );
};
